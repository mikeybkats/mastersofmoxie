...
if(isset($_POST['domain']))
{
	...
	if(strlen($punny_domain) > 0)
	{
		...
		if($whois != 'disable') {
				if($whois_custom_url != ''){
		    		$whois = $whois_custom_url;
			    }else{
			    	$whois = get_permalink($whois)."?&domain=$domain";
			    }

			$whois_link = "<a href='".$whois."' ".$whois_new_tab." ><button id='whois' class='btn btn-danger btn-xs pull-right whois-btn'>WHOIS</button></a>";
		}else{
			$whois_link = '';
		}
...
if(count($check_ex) == 2){
	...
	$ex_name = $check_ex[0];
			...
			$additional_button = "<a href='javascript:void(0)' onclick='submitform_$dom_$ex_name()' ><button id='buy' class='btn btn-success btn-xs pull-right order-btn'>$additional_button_name</button></a>";
		}elseif($integration == 'woocommerce'){
			if($show_price){
				$show_price = '- '.wdc_display_price($domain).'/year';
			}
			$additional_button = "<a href='?&add-to-cart=$additional_button_link&domain=$domain' id='buy' class='btn btn-success btn-xs pull-right order-btn' $buy_new_tab >$additional_button_name $show_price</a>";
			...
			if(!$additional_button_name == '' AND !$additional_button_link == ''){
				...
				$additional_button = "<a id='buy' class='btn btn-success btn-xs pull-right order-btn' href='$additional_button_links' $buy_new_tab >$additional_button_name</a>";
			}else{
				$additional_button = '';
			}
		}else{
		...
		if ($available->status == 1) {
				$result = array('status'=>1,
								...
								'text'=> 	'<div class="callout callout-success alert-success clearfix available">
											<div class="col-xs-10" style="padding-left:1px;text-align:left;">
											<i class="glyphicon glyphicon-ok" style="margin-right:1px;"></i> '.$custom_found_result_text.' </div>
...
function wdc_options() {
    ...
    'type' => 'select',
    'options' => $whois_page,
    'desc' => 'Enable or disable whois button if domain not available, make sure you put shortcode <strong>[wpdomainwhois]</strong> in whois page.',
    'default' => 'disable',
	) );

	$generaltab->createOption(array(
    'name' => 'Whois Custom URL',
    'id' => 'whois_custom_url',
    'type' => 'text',
    'desc' => 'Use custom URL instead whois page'
));
	...
	$generaltab->createOption( array(
    'name' => 'Show Domain Price in Button?',
    'id' => 'show_price',
    'type' => 'checkbox',
    'default' => false,
    'desc' => '<em>WooCommerce Integration Only.</em>'
	) );
