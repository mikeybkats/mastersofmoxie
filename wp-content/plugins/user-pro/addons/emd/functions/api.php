...
class userpro_ed_api {
	...
	// verify admins
	function verify_admins(){
		if ( !get_option('userpro_admins_got_verified') ) {
		$users = get_users('role=administrator');
		foreach($users as $user){
			update_user_meta($user->ID, 'userpro_verified', 1);
		}
		update_option('userpro_admins_got_verified', 1);
		}
	}
	...
	function print_field($field, $user_id) {
		...
		<div class="emd-user-left"><?php echo $userpro->field_label($field); ?></div>
...
function collageusers( $args ){
	...
	// Loop thru options
	function loop_options( $key, $num_range=null ){
...
$userpro_emd = new userpro_ed_api();
