...
if(userpro_get_option('enable_connect') == 'y')
	add_action('userpro_after_profile_head','userpro_connect', 99);

function userpro_connect( $args)
{			
		global $userpro;
		$user_id = $args['user_id'];


if ( in_array($args['template'], array('view','following','followers','connections') )  && !isset($args['no_style']) ){
...
?>
<a href="<?php echo $userpro->permalink($user_id, 'connections','userpro_connections'); ?>" class="userpro-count-link"><?php echo $userpro->connetions_count($user_id); ?></a></div>
...
function userpro_avoid_conflict(){
	add_action('wp_title' , 'userpro_replace_wp_title' ,999 );
}
